---
title: "How does the daily occupancy rate of ON shelters change under the shock of COVID-19?"
author: "JIN QIU"
...
This research aims at discovering the change of the situations of daily
shelters in ON due to the shock of COVID-19. The analysis is based on
...
finding is that the average occupancy rate has decreased in 2020, while the
situation of overwhelmed shelter is even more severe.

# Introduction

In year 2020, due to the influence of COVID-19, the Ontario government has
required everyone to keep social distancing and reduce the frequency
...
From the analysis, I noticed that the number of shelters that are in
operation this year is less than the number of last year(2019),
...
of a overwhelmed shelter has been happening since last year, the highest
occupancy rate in ON this year for a single shelter has arrived 4.05,
which is almost 3 times of that in last year. This indicates that the use ratio
...
During the process of analyzing the occupancy rate, we are using the
...
research in the middle of 2020, the data I am using only covered the
...
ratio might be underestimated with using the current data. Another
possible error is, when I am doing the research, neither the data from
2020 or the data from 2019 is selected randomly, I simply used all
...
datastore_resources <- filter(resources, tolower(format) %in% c('csv', 'geojson')) # Download the data from "opendata" Toronto website.

data20 <- filter(datastore_resources, row_number()==1) %>% get_resource()  # Get our data for 2020
data20


data19 <- filter(datastore_resources, row_number()==2) %>% get_resource()  # Get our data for 2019, we will need it for comparison.
data19
```
...
shelters on running(because of the shortage workforce or other reasons).


# Futhermore, how does the occupancy rate change due to the COVID-19, compared to that in year 2019?

```{r plots, echo=FALSE}
ratio19= data19$OCCUPANCY/data19$CAPACITY # This is how we calculated the occupancy rate
...
This two plots show that there might be some positive linear relationship
between capacity and occupancy. We can conclude that the variance of the
data in 2020 is larger than that of 2019. This means, the differences in the situation 
of different shelters in 2020 is largers.
...
2019. In specific, the histogram for year 2020 is hardly right-skewed,
while we could say the histogram for year 2019 is only slightly right-skewed.
...
even if we are only looking at the data in year 2019, we can say that
...
Other than those factors that we have discussed at the very beginning of this 
report, the process of data collection for year 2019 might also be biased. As
we know, the winter in 2019 is actually warmer than that in the previous years. That might
be a possible reason that lowers the average occupancy rate for shelters in 2019.
The analysis will be more accurate if we could take more years into account or analyzing the data
with a partition of the seasons, and that is what I am going to do for the further research.
